...
public partial class TweakViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private string _statusIcon = "Help";
    ...
    private void Tweak_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        try
        {
            if (e.PropertyName == nameof(ITweak.Status))
            {
                ...
                if (dispatcher.HasThreadAccess) UpdateStatusUI();
        ...
        try
        {
            switch (_tweak.Status)
            {
                case TweakStatus.Optimized:
                    ...
                    StatusIcon = "Accept";
                    StatusColor = new SolidColorBrush(ColorHelper.FromArgb(0xff, 0x0f, 0x7b, 0x0f)); // Verde Escuro
                    break;
                case TweakStatus.Default:
                    StatusText = Resources.Status_Default;
                    StatusIcon = "Cancel";
                    StatusColor = new SolidColorBrush(ColorHelper.FromArgb(0xff, 0xc4, 0x2b, 0x1c)); // Vermelho
                    break;
                case TweakStatus.Modified:
                    StatusText = Resources.Status_Modified;
                    StatusIcon = "Edit";
                    StatusColor = new SolidColorBrush(ColorHelper.FromArgb(0xff, 202, 80, 16)); // Laranja
                    break;
                default:
