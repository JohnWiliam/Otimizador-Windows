...
public class TweakService
{
    ...
    public async Task RefreshStatusesAsync()
    {
        await Task.Run(() =>
        {
            ...
            Parallel.ForEach(Tweaks, parallelOptions, tweak =>
            {
    ...
    private void AddNetworkTweaks()
    {
        ...
        Tweaks.Add(new CustomTweak("N4", TweakCategory.Network, Resources.N4_Title,
            Resources.N4_Desc + " (Proteção: RSS só é desativado em CPUs de núcleo único; em hardware moderno ele permanece habilitado.)",
            () =>
            {
                if (!ShouldDisableReceiveSideScaling())
                {
                    Logger.Log("N4 ignorado: RSS é benéfico em hardware moderno com múltiplos núcleos.", "WARNING");
                    return false;
                }

                CommandHelper.RunCommand("netsh", "int tcp set global rss=disabled");
                return true;
            },
    ...
    private void AddCustomTweaks()
    {
        ...
        // SE2: Prefetch
        Tweaks.Add(new CustomTweak("SE2", TweakCategory.Tweaks, Resources.SE2_Title,
            Resources.SE2_Desc + " (Proteção: só desativa em unidade de sistema SSD; em HDD o Prefetch permanece ativo.)",
            () =>
            {
                if (!IsSystemDriveLikelySsd())
                {
                    Logger.Log("SE2 ignorado: unidade de sistema não parece ser SSD ou não pôde ser detectada com segurança.", "WARNING");
                    return false;
                }
            ...
            () =>
            {
                var value = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management\PrefetchParameters", "EnablePrefetcher", -1);
                return value is int i && i == 0;
            }
        ));
    }

    private static bool ShouldDisableReceiveSideScaling() => Environment.ProcessorCount <= 1;

    private static bool IsSystemDriveLikelySsd()
    {
        try
        {
            ...
            string driveLetter = systemRoot.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar).TrimEnd(':');
            string script = $"$partition = Get-Partition -DriveLetter '{driveLetter}' -ErrorAction Stop; " +
                            "$disk = $partition | Get-Disk -ErrorAction Stop; " +
                            "$physical = Get-PhysicalDisk | Where-Object { $_.DeviceId -eq $disk.Number }; " +
                            "$physical.MediaType";

            string mediaType = CommandHelper.RunCommand("powershell", $"-NoProfile -ExecutionPolicy Bypass -Command {ToPowerShellLiteral(script)}");
            return mediaType.Contains("SSD", StringComparison.OrdinalIgnoreCase);
        }
        catch (Exception ex)
        {
            Logger.Log($"Falha ao detectar tipo da unidade de sistema: {ex.Message}", "WARNING");
            return false;
        }
    }
