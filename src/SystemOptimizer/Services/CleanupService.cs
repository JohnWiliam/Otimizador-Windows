...
public class CleanupService
{
    ...
    public CleanupService(CleanupExecutionEngine executionEngine, IEnumerable<ICleanupTargetProvider> targetProviders)
    {
        ...
        var providersByCategory = targetProviders
        ...
        {
            [UserTempKey] = new(UserTempKey, Resources.Label_TempFiles ?? "User Temp", GetTargetsFor(UserTempKey)),
            [SystemTempKey] = new(SystemTempKey, Resources.Label_SystemTemp ?? "System Temp", GetTargetsFor(SystemTempKey)),
            [PrefetchKey] = new(PrefetchKey, "Prefetch", GetTargetsFor(PrefetchKey)),
            [BrowserCacheKey] = new(BrowserCacheKey, Resources.Label_BrowserCache ?? "Browser Cache", GetTargetsFor(BrowserCacheKey)),
            [DnsKey] = new(DnsKey, "DNS Cache", GetTargetsFor(DnsKey)),
            [WindowsUpdateKey] = new(WindowsUpdateKey, "Windows Update", GetTargetsFor(WindowsUpdateKey)),
            [RecycleBinKey] = new(RecycleBinKey, "Lixeira", GetTargetsFor(RecycleBinKey))
        };

        List<CleanupTarget> GetTargetsFor(string categoryKey)
            => providersByCategory.TryGetValue(categoryKey, out var targets) ? targets : [];
    }
