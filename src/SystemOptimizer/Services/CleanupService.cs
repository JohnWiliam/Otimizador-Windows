...
public class CleanupService
{
    ...
    public CleanupService(CleanupExecutionEngine executionEngine, IEnumerable<ICleanupTargetProvider> targetProviders)
    {
        ...
        var targetsByCategory = targetProviders
            ...
            .ToDictionary(
                group => group.Key,
                group => (IReadOnlyList<CleanupTarget>)group.SelectMany(provider => provider.GetTargets()).ToList(),
                StringComparer.OrdinalIgnoreCase);

        _categoryMap = new(StringComparer.OrdinalIgnoreCase)
        {
            [UserTempKey] = new(UserTempKey, Resources.Label_TempFiles ?? "User Temp", GetTargetsForCategory(targetsByCategory, UserTempKey)),
            [SystemTempKey] = new(SystemTempKey, Resources.Label_SystemTemp ?? "System Temp", GetTargetsForCategory(targetsByCategory, SystemTempKey)),
            [PrefetchKey] = new(PrefetchKey, "Prefetch", GetTargetsForCategory(targetsByCategory, PrefetchKey)),
            [BrowserCacheKey] = new(BrowserCacheKey, Resources.Label_BrowserCache ?? "Browser Cache", GetTargetsForCategory(targetsByCategory, BrowserCacheKey)),
            [DnsKey] = new(DnsKey, "DNS Cache", GetTargetsForCategory(targetsByCategory, DnsKey)),
            [WindowsUpdateKey] = new(WindowsUpdateKey, "Windows Update", GetTargetsForCategory(targetsByCategory, WindowsUpdateKey)),
            [RecycleBinKey] = new(RecycleBinKey, "Lixeira", GetTargetsForCategory(targetsByCategory, RecycleBinKey))
        };
    }

    private static IReadOnlyList<CleanupTarget> GetTargetsForCategory(
        IReadOnlyDictionary<string, IReadOnlyList<CleanupTarget>> targetsByCategory,
        string categoryKey)
    {
        return targetsByCategory.TryGetValue(categoryKey, out var targets) ? targets : [];
    }
