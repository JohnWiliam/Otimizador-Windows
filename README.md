...
O **Otimizador de Sistema** é uma aplicação moderna, portátil e robusta desenvolvida em **C# 14, .NET 10 e WinUI 3** para ajustar, limpar e acelerar o Windows. Ele utiliza **Windows App SDK/WinUI 3** com **Fluent 2 UI** e **Mica** para oferecer uma interface moderna e nativa, garantindo uma experiência nativa no Windows 11.
...
* **Instalação via MSIX**: O sistema baixa o pacote `.msix` e abre o instalador nativo do Windows.
...
3.  O pacote final estará em: `Build\*.msix`.
    * *Nota: o pacote é gerado sem assinatura para fluxos de sideload/desenvolvimento.*
...
**System Optimizer** is a modern, portable, and robust application built in **C# 14, .NET 10, and WinUI 3** to tweak, clean, and accelerate Windows. It uses the **Windows App SDK/WinUI 3** with **Fluent 2 UI** and **Mica** to deliver a modern native interface, ensuring a native feel on Windows 11.
...
* **MSIX Install**: Downloads the `.msix` package and opens the native Windows installer.
...
* 📜 **build.ps1**: Automated script to compile and package the WinUI 3 application as MSIX.
...
3.  The final package will be located at: `Build\*.msix`.
    * *Note: the package is generated unsigned for sideload/development workflows.*
